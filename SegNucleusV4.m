...
minCellSize = 70; % change this according to magnification used
...
[out.mtch out.nosm] = matchpatch_H2B(pimg1,pimg2,ptch1,ptch2);
out.upd_dismat = {};
out.data_until = [];
out.mtable = [];
...
%         fullpth=fullfile(readpth,wells{w},sites{s});
        ...
        for i=1:length(nuc_lst)
            ...
            cyc_bw = imerode(cyc_bw,strel('disk',7));
            
            lbl=bwlabel(cyc_bw);
            lbl = imdilate(lbl,strel('disk',7));
...
function tracknexport(segtpt,fullpth,sz)
            ...
            save(fullfile(fullpth,'SegQuantDataH2B.mat'),'tpt');
            
            imwritestack(uint16(mpimg),clmp,fullfile(fullpth,'TrackingH2B.tif'));
