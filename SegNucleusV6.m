...
readpth = '/Volumes/quantcellbio$/SHERIFF/sortedHeLa/';
...
%         seg_track(readpth,w,s);
        ...
        display(sprintf('Well:%s Pos:%s',wells{w},sites{s}));
...
% %             cyc_bw = imerode(cyc_bw,strel('disk',3));
%             lbl=bwlabel(cyc_bw);
            lbl = imdilate(lbl,strel('disk',Er));
            %              lbl=imclose(lbl,strel('disk',10));
            lbl = imfill(lbl,'holes');
            %             figure(1029),subplot(2,1,1),imagesc(lbl);
%             figure(122), imagesc(lbl);
...
function tracknexport(segtpt,fullpth,sz)
            ...
            for i=1:length(segtpt)-1 %Start matching for consecutive time points of same channel                
                bf = i;
                af = i+1;
                display(sprintf('Matching: Image: %d vs Image: %d',bf,af));
            ...
            mkdir(fullpth);
            save(fullfile(fullpth,'SegQuantData75_Er12.mat'),'tpt');
            outfilename = fullfile(fullpth,'Tracking75_Er12.tif');
